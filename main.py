...
name1 = 'old'  # Удобнее иметь фиксированные имена файлов, чем вводить их каждый раз
name2 = 'new'
...
while True:
    print("Введите имя папки выбраннного ученика")
    folder = input()

    with open('data/' + folder + '/' + name1 + '.csv', 'r', encoding='UTF-8') as f:
        file1 = f.read().split('\n')

    with open('data/' + folder + '/' + name2 + '.csv', 'r', encoding='UTF-8') as f:
        file2 = f.read().split('\n')
